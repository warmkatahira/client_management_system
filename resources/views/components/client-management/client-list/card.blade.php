<div class="w-full grid grid-cols-12 gap-4">
    @foreach($clients as $client)
        <div class="col-span-3 border border-gray-300 rounded-2xl shadow-md p-4 flex flex-col items-center {{ $client->client_status_id == ClientStatusEnum::ACTIVE ? 'bg-white' : 'bg-gray-300' }} transition duration-300 ease-in-out transform hover:-translate-y-2 hover:shadow-xl">
            ...
            <div class="w-full h-40 flex justify-center items-center {{ $client->client_status_id == ClientStatusEnum::ACTIVE ? 'bg-gray-100' : 'bg-gray-300' }} rounded-xl overflow-hidden">
            ...
            <div class="flex gap-5 mt-4">
                ...
                @if($client->client_hp)
                    <a href="{{ $client->client_hp }}" target="_blank" rel="noopener noreferrer" class="btn bg-theme-main text-white py-1 px-5 rounded-full tippy_jump_hp">HP</a>
                @endif
            </div>
        </div>
    @endforeach
